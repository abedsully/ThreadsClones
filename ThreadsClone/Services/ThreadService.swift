...
struct ThreadService {
    ...
    static func fetchThreadd(threadId: String) async throws -> Thread {
...
extension ThreadService {
    ...
    static func unlikeThread(_ thread: Thread) async throws {
        ...
        async let _ = FirestoreConstants.UserCollection.document(uid).collection("user-likes").document(thread.id).delete()
        async let _ = try await threadRef.updateData(["likes": thread.likes - 1])
    }
